...
function OrderCard({sale_id, sale_date, sale_status, shipped_date, received_date, cancelled_date, total, return_request_date}) {
                    ...
                    <div className="flex flex-col w-1/2 ">
                        ...
                        <div className="flex md:flex-row flex-col gap-5 h-full items-start whitespace-nowrap">
                            ...
                            <div className="flex flex-col justify-start items-start pl-2 border-l-2 h-full">
                                ...
                                {!(sale_status === "packed" || sale_status === "cancelled" || sale_status === "processing order" || sale_status === "processing return") && <p className="text-xs font-normal text-start">Shipped Date: <span className="text-xs font-semibold">{shipped_date}</span></p>}
                                ...
                                {(sale_status ===  "processing return") && <p className="text-xs font-normal text-start">Return Request Date: <span className="text-xs font-semibold">{return_request_date}</span></p>}
